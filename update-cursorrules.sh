...
# Update .cursorrules with current project information
# This script should be run after any significant project changes

set -e

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CURSORRULES_FILE="$PROJECT_ROOT/.cursorrules"
TEMP_FILE="$PROJECT_ROOT/.cursorrules.tmp"

echo "🔄 Updating .cursorrules with current project information..."

# Get current date
CURRENT_DATE=$(date "+%Y-%m-%d %H:%M:%S")

# Count apps in each category (only direct subdirectories, not node_modules)
APPS_COUNT=0
APPLICATIONS_COUNT=0
GAMES_COUNT=0
SIMULATIONS_COUNT=0

for category in "Applications" "Games" "Simulations"; do
    if [ -d "$category" ]; then
        for app_dir in "$category"/*; do
            if [ -d "$app_dir" ] && [ -f "$app_dir/package.json" ]; then
                case "$category" in
                    "Applications") APPLICATIONS_COUNT=$((APPLICATIONS_COUNT + 1)) ;;
                    "Games") GAMES_COUNT=$((GAMES_COUNT + 1)) ;;
                    "Simulations") SIMULATIONS_COUNT=$((SIMULATIONS_COUNT + 1)) ;;
                esac
    ...
    if [ -d "$category" ]; then
        for app_dir in "$category"/*; do
            if [ -d "$app_dir" ] && [ -f "$app_dir/package.json" ]; then
                ...
                APP_LIST="$APP_LIST- $category: $app_name\n"
            fi
        done
    fi
done

# Check if git is available and get repo info
GIT_INFO=""
if command -v git >/dev/null 2>&1 && git rev-parse --git-dir >/dev/null 2>&1; then
    COMMIT_HASH=$(git rev-parse --short HEAD 2>/dev/null || echo "unknown")
    BRANCH_NAME=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")
    GIT_INFO="Last updated: $CURRENT_DATE (commit: $COMMIT_HASH, branch: $BRANCH_NAME)"
else
    GIT_INFO="Last updated: $CURRENT_DATE"
fi

# Create updated .cursorrules file
cat > "$TEMP_FILE" << EOF
# Figma Make Competition Portfolio - AI Assistant Rules
# $GIT_INFO
# Total Apps: $APPS_COUNT ($APPLICATIONS_COUNT Applications, $GAMES_COUNT Games, $SIMULATIONS_COUNT Simulations)

## Project Overview
This is a unified portfolio hosting solution for multiple React applications organized into three categories:
- **Applications**: Productivity tools and utilities ($APPLICATIONS_COUNT apps)
- **Games**: Interactive games and prototypes ($GAMES_COUNT apps)
- **Simulations**: Scientific and mathematical simulations ($SIMULATIONS_COUNT apps)
...
├── Applications/          # Productivity apps ($APPLICATIONS_COUNT apps)
├── Games/                 # Interactive games ($GAMES_COUNT apps)
└── Simulations/          # Scientific simulations ($SIMULATIONS_COUNT apps)
\`\`\`
...
echo "✅ .cursorrules updated successfully!"
echo "📊 Project stats:"
echo "   - Total apps: $APPS_COUNT"
echo "   - Applications: $APPLICATIONS_COUNT"
echo "   - Games: $GAMES_COUNT"
echo "   - Simulations: $SIMULATIONS_COUNT"
echo "🕒 $GIT_INFO"
